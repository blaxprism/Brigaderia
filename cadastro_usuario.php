...
<head>
	...
	<link rel="stylesheet" href="estilos/style.css"> <!-- Inclui o arquivo CSS para estilização -->
</head>
<body>
	<main>
		<header>
			<div id="linha_header">
				<a href="index.html" id="logo_link">
					<img src="https://img.freepik.com/icones-gratis/bombom_318-797825.jpg?w=2000" id="logo">
				</a>
				<nav>
					<a href="login.html" class="header">Login</a>
					<a href="estoque.html" class="header">Estoque</a>
					<a href="pedidos.html" class="header">Pedidos</a>
					<a href="cadastro_administrador.html" class="header">Cadastro de Administrador</a>
					<a href="cadastro_usuario.html" class="header">Cadastro de Usuário</a>
				</nav>	
			</div>
		</header>
		<section>
			<?php
				...
				if (!empty($validade_campos) && is_array($validade_campos)) {
					...
					if($mensagemErro == false){
						if ($consultas->UsuarioExiste($connection, $email)) {
							echo "Usuário já cadastrado";
						}else{	
							$resultado = $cadastro->CadastroUsuario($connection, $nome, $email, $senha, $celular, $telefone);
							echo $resultado;
						}
					}else{
						print($mensagemErro);
			...
			?>
			<br><br>
			<a href="cadastro_usuario.html" style="border-style: solid; border-color: #ffffff; background-color: #ffffff; color: #000000;">Retorne para a página de cadastro</a>
		</section>                
	</main>
</body>
</html>